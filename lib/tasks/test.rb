...
namespace :test do
  ...
  task :quick do
    fail 'tests failed' unless system 'bundle exec strainer test -e kitchen'
  end
  ...
  namespace :debug do
    ...
    task :quick do
      fail 'tests failed' \
        unless system 'bundle exec strainer test -e kitchen --log-level=debug'
    end

    desc 'Run all tests, including kitchen, with debugging enabled'
    task :all do
      fail 'tests failed' \
        unless system 'bundle exec strainer test --log-level=debug'
    end
  end
end
